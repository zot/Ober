...
public class OberSwtLayout extends Layout {
	...
	public void addControl(Control control, int index, float position) {
		...
		positions[0] = 0;
		control.setParent(viewer.component);
	...
	protected void layout(Composite comp, boolean flushCache) {
		for (int index = 0; index < controls.size(); index++) {
			...
			((Control)controls.get(index)).setBounds(1 + (vertical ? 0 : Math.round(vs.x * position)), 1 + (vertical ? Math.round(vs.y * position) : 0), Math.round(vertical ? vs.x : vs.x * size) - 2, Math.round(vertical ? vs.y * size : vs.y) - 2);
		}
	}
	...
	public void constrain(Point displayPoint, OberSwtDragger widget) {
		Shell top = widget.getShell();
		Point splitterPoint = widget.viewer.wrapper.getParent().toControl(displayPoint);
		Point widgetLocation = widget.viewer.wrapper.getParent().toControl(widget.toDisplay(widget.getLocation()));
		Control target = findComponentAt(top, displayPoint);
		...
		if (vertical) {
			widget.activeTracker.setLocation(widgetLocation.x, Math.min(splitterPoint.y, targetParent.getBounds().height - widget.activeTracker.getBounds().height));
		} else {
			widget.activeTracker.setLocation(Math.min(splitterPoint.x, widget.activeTracker.getParent().getBounds().width - widget.activeTracker.getBounds().width), widgetLocation.y);
		}
	}
}
